  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(
        new URL(`/calendario?error=oauth_failed`, request.url),
      );
    }
    ...
    if (!code || !state) {
      return NextResponse.redirect(
        new URL(`/calendario?error=invalid_callback`, request.url),
      );
    }
    ...
    if (!user) {
      return NextResponse.redirect(
        new URL(`/calendario?error=unauthorized`, request.url),
      );
    }
    ...
    try {
      ...
      return NextResponse.redirect(
        new URL(`/calendario?success=google_connected`, request.url),
      );
    } catch (tokenError) {
      console.error("Error exchanging tokens:", tokenError);
      return NextResponse.redirect(
        new URL(`/calendario?error=token_exchange_failed`, request.url),
      );
    }
  } catch (error) {
    console.error("Error in Google Calendar callback:", error);
    return NextResponse.redirect(
      new URL(`/calendario?error=callback_failed`, request.url),
    );
  }
}
