...
import { uploadEnhancedLightingImageToS3 } from "~/app/actions/enhance-lighting-image";
...
/**
 * POST: Save lighting-enhanced image to S3 and database when user confirms
 */
export async function POST(
  ...
  try {
    ...
    if (!session?.user?.id) {
      console.error("Unauthorized access to save enhanced lighting image");
    ...
    if (
      ...
      console.error("Missing required fields for enhanced lighting image save");
      return Response.json(
        {
          error:
            "enhancedImageBase64, referenceNumber, and currentImageOrder are required",
        },
        { status: 400 },
      );
    }
    ...
    console.log("Saving enhanced lighting image:", {
      ...
      imageDataLength: data.enhancedImageBase64.length,
    });
    ...
    // Note: We filter out AI-enhanced/renovated/blurred images to ensure we get the actual original
    ...
    console.log("🎯 Enhanced lighting image save API - Starting process:", {
    ...
    try {
      console.log("📤 Calling uploadEnhancedLightingImageToS3 with:", {
        base64Length: data.enhancedImageBase64.length,
        correctPropertyId: correctPropertyId.toString(),
        referenceNumber: data.referenceNumber,
        newImageOrder,
      ...
      const propertyImage = await uploadEnhancedLightingImageToS3(
        data.enhancedImageBase64,
        correctPropertyId,
        data.referenceNumber,
        newImageOrder,
      ...
      console.log("🎉 Enhanced lighting image successfully saved:", {
      ...
      return Response.json({
        ...
        message: "Enhanced lighting image saved successfully",
      });
    } catch (uploadError) {
      console.error("❌ DETAILED ERROR saving enhanced lighting image to S3/DB:", {
        error: uploadError,
        message:
        ...
        data: {
          base64Length: data.enhancedImageBase64?.length ?? 0,
          referenceNumber: data.referenceNumber,
          currentImageOrder: data.currentImageOrder,
          newImageOrder,
      ...
      return Response.json(
        {
          error: `Failed to save enhanced lighting image: ${uploadError instanceof Error ? uploadError.message : "Unknown error"}`,
          details:
    ...
    console.error("❌ Enhanced lighting image save API error:", error);

    if (error instanceof Error) {
      return Response.json(
        {
          error: `Failed to save enhanced lighting image: ${error.message}`,
        },
        { status: 500 },
      );
    }
    ...
    return Response.json(
      { error: "Failed to save enhanced lighting image" },
      { status: 500 },
    );
  }
}
