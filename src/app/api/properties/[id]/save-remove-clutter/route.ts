...
import { uploadRemoveClutterImageToS3 } from "~/app/actions/remove-clutter-image";
...
/**
 * POST: Save decluttered image to S3 and database when user confirms
 */
export async function POST(
  ...
  try {
    ...
    if (!session?.user?.id) {
      console.error("Unauthorized access to save decluttered image");
    ...
    if (
      !data.declutteredImageBase64 ||
      !data.referenceNumber ||
      !data.currentImageOrder
    ) {
      console.error("Missing required fields for decluttered image save");
      return Response.json(
        {
          error:
            "declutteredImageBase64, referenceNumber, and currentImageOrder are required",
        },
        { status: 400 },
      );
    }
    ...
    console.log("Saving decluttered image:", {
      ...
      imageDataLength: data.declutteredImageBase64.length,
    });
    ...
    // Note: We filter out AI-enhanced/renovated/decluttered images to ensure we get the actual original
    ...
    console.log("🎯 Decluttered image save API - Starting process:", {
    ...
    try {
      console.log("📤 Calling uploadRemoveClutterImageToS3 with:", {
        base64Length: data.declutteredImageBase64.length,
        correctPropertyId: correctPropertyId.toString(),
        referenceNumber: data.referenceNumber,
        newImageOrder,
      ...
      const propertyImage = await uploadRemoveClutterImageToS3(
        data.declutteredImageBase64,
        correctPropertyId,
        data.referenceNumber,
        newImageOrder,
      ...
      console.log("🎉 Decluttered image successfully saved:", {
      ...
      return Response.json({
        ...
        message: "Decluttered image saved successfully",
      });
    } catch (uploadError) {
      console.error("❌ DETAILED ERROR saving decluttered image to S3/DB:", {
        error: uploadError,
        message:
        ...
        data: {
          base64Length: data.declutteredImageBase64?.length ?? 0,
          referenceNumber: data.referenceNumber,
          currentImageOrder: data.currentImageOrder,
          newImageOrder,
      ...
      return Response.json(
        { error: "Failed to save decluttered image to S3 or database" },
        { status: 500 },
      );
    }
  } catch (error) {
    console.error("Save decluttered image API error:", error);
    return Response.json(
      { error: "Failed to save decluttered image" },
      { status: 500 },
    );
  }
}
