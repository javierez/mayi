...
/**
 * Upload blurred faces image to S3 and create database record
 * Follows the same pattern as renovated images but for blurred faces
 */
export async function uploadBlurredFacesImageToS3(
  ...
  try {
    console.log("🔄 Starting uploadBlurredFacesImageToS3:", {
      base64Length: blurredImageBase64.length,
    ...
    const imageBuffer = base64ToBuffer(blurredImageBase64);
    console.log("✅ Buffer created, size:", imageBuffer.length);

    // 2. Generate the S3 key for the blurred image
    ...
    // 5. Create image tag
    const imageTag = "ai_blur_faces";
    console.log("🏷️ Image tag:", imageTag);

    // 6. Create database record with ai_blur_faces tag
    ...
    > = {
      ...
      s3key,
      imageOrder,
      imageTag, // Mark as AI blurred faces
    ...
    if (!result) {
      ...
      throw new Error("Failed to create blurred faces property image record");
    }

    // 7. Fetch the complete image record
    const propertyImage = await getPropertyImageById(result.propertyImageId);
    if (!propertyImage) {
      throw new Error("Failed to fetch created blurred faces property image");
    }
    ...
    console.log("Successfully uploaded blurred faces image:", {
    ...
    console.error("❌ CRITICAL ERROR in uploadBlurredFacesImageToS3:", {
      error,
    ...
    throw new Error(
      `Failed to upload blurred faces image: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
