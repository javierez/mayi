...
/**
 * Upload 3D rendered image to S3 and create database record
 * Follows the same pattern as renovated/blurred images but for 3D renders
 */
export async function uploadRender3dImageToS3(
  ...
  try {
    console.log("🔄 Starting uploadRender3dImageToS3:", {
      base64Length: render3dImageBase64.length,
    ...
    const imageBuffer = base64ToBuffer(render3dImageBase64);
    console.log("✅ Buffer created, size:", imageBuffer.length);

    // 2. Generate the S3 key for the 3D rendered image
    ...
    // 5. Create image tag
    const imageTag = "ai_render_3d";
    console.log("🏷️ Image tag:", imageTag);

    // 6. Create database record with ai_render_3d tag
    ...
    if (!result) {
      ...
      throw new Error("Failed to create 3D render property image record");
    }

    // 7. Fetch the complete image record
    const propertyImage = await getPropertyImageById(result.propertyImageId);
    if (!propertyImage) {
      throw new Error("Failed to fetch created 3D render property image");
    }
    ...
    console.log("Successfully uploaded 3D render image:", {
    ...
    console.error("❌ CRITICAL ERROR in uploadRender3dImageToS3:", {
      error,
    ...
    throw new Error(
      `Failed to upload 3D render image: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
