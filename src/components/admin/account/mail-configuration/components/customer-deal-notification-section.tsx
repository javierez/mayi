...
import type { CustomerDealNotificationSettings } from "../types";
import { CustomerNotificationOptionRow } from "./customer-notification-option-row";

interface CustomerDealNotificationSectionProps {
  settings: CustomerDealNotificationSettings;
  onToggleEmail: (optionKey: keyof CustomerDealNotificationSettings) => void;
  onToggleSMS: (optionKey: keyof CustomerDealNotificationSettings) => void;
}

export function CustomerDealNotificationSection({
  settings,
  onToggleEmail,
  onToggleSMS,
}: CustomerDealNotificationSectionProps) {
          ...
          <CardHeader className="cursor-pointer py-4 hover:bg-gray-50/50">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Handshake className="mr-2 h-4 w-4 text-gray-600" />
                <div>
                  <CardTitle className="text-sm font-medium text-gray-900">
                    Notificaciones de Operaciones
                  </CardTitle>
                  <CardDescription className="mt-0.5 text-xs text-gray-500">
                    Notificaciones sobre ofertas, operaciones y pagos
                  </CardDescription>
                </div>
              </div>
          ...
          <CardContent className="pt-0">
            <div className="divide-y divide-gray-100">
              {(
                Object.keys(
                  settings,
                ) as Array<keyof CustomerDealNotificationSettings>
              ).map((key) => (
